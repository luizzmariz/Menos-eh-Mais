...
public class CylinderGenerator : MonoBehaviour
{
    ...
    //Alterations -- Mariz
    public int cylinderRadius = 3;
    public int cylinderIterations = 10;

    void Start()
    {
        Mesh blabla = GenerateCylinder();

        this.gameObject.GetComponent<MeshFilter>().mesh = blabla;
		this.gameObject.GetComponent<MeshCollider>().sharedMesh = blabla;
        this.gameObject.GetComponent<MeshCollider>().convex = true;
		this.gameObject.GetComponent<MeshCollider>().isTrigger = true;
        this.gameObject.transform.rotation = new Quaternion(-0.707106829f,0,0,0.707106829f);
